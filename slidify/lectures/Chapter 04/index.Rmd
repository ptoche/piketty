...
df_4_4$interaction <- interaction(df_4_4$variable, df_4_4$Category, sep = " ")
n <- nPlot(data = df_4_4, value ~ Year, group = 'interaction', type = 'lineChart') 
...
df_4_10$variable <- factor(df_4_10$variable, levels = c("Agricultural land", "Housing", "Other domestic capital", "Slaves", "Net foreign capital"), ordered = TRUE)  # re-order the factors
df_4_10 <- df_4_10[order(df_4_10$variable), ]  # re-order the variables
...
n$chart(showControls = FALSE)
n$chart(useInteractiveGuideline = TRUE)
n$chart(color = c('#E41A1C', '#377EB8', '#4DAF4A', '#FF7F00', '#984EA3'))
n$addParams(height = 450, width = 900)
...
# reorder variables for stacking
df_4_11$variable <- factor(df_4_11$variable, levels = c("Agricultural land", "Housing", "Other domestic capital", "Slaves", "Net foreign capital"), ordered = TRUE)  # re-order the factors
df_4_11 <- df_4_11[order(df_4_11$variable), ]  # re-order the variables
