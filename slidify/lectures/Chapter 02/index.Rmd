...
```{r 'Figure_2_T4a_rCharts',  message = FALSE, warning = FALSE, error = FALSE, echo = FALSE, tidy = FALSE, cache = FALSE}  
require(rCharts)
load("../../data/df_2_T4.Rda") 
# round data for rChart tooltip display
df <- subset(df_2_T4, Country == "France")
df$value <- round(df$value, 3)
n <- nPlot(data = df, value ~ Year, group = 'variable', type = 'stackedAreaChart') 
n$yAxis(axisLabel = 'Share in employment (% of total)')
n$chart(margin = list(left = 80)) 
n$yAxis(tickFormat = "#! function(d) {return Math.round(d*100*100)/100 + '%'} !#")
n$xAxis(axisLabel = NULL)
n$chart(style = 'stack')
...
# n$chart(color = c('#EFF3FF', '#BDD7E7', '#6BAED6', '#2171B5')) # computer
n$addParams(height = 450, width = 900)
n$save('figures/Figure_2_T4a.html', standalone = TRUE)
```
...
```{r 'Figure_2_T4b_rCharts',  message = FALSE, warning = FALSE, error = FALSE, echo = FALSE, tidy = FALSE, cache = FALSE}  
require(rCharts)
load("../../data/df_2_T4.Rda") 
# round data for rChart tooltip display
df <- subset(df_2_T4, Country == "United States")
df$value <- round(df$value, 3)
n <- nPlot(data = df, value ~ Year, group = 'variable', type = 'stackedAreaChart') 
n$yAxis(axisLabel = 'Share in employment (% of total)')
n$chart(margin = list(left = 80)) 
...
# n$chart(color = c('#EFF3FF', '#BDD7E7', '#6BAED6', '#2171B5')) # computer
n$addParams(height = 450, width = 900)
n$save('figures/Figure_2_T4b.html', standalone = TRUE)
```
