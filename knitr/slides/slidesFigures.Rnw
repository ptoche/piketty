...
%%% minor updates to keep up with ggplot2 updated syntax: August 2018
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
opts_chunk$set(results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE, concordance = TRUE, cache = TRUE)

# Set smaller font size for chunks (for code display):
opts_chunk$set(size = 'footnotesize')

### Set a font for slides and plots
library(sysfonts)
font.add("xkcd", regular = "xkcd.ttf")  # xkcd fonts to illustrate principle

### Set Cairo fonts if Cairo package is used to export figures, e.g. CairoJPEG
library(Cairo)  # not currently used
mainfont <- "Garamond"
# mainfont <- "xkcd"  # xkcd fonts to to illustrate principle
...
# many other output formats are possible, e.g. postscript, svg, tiff, etc.
# CairoJPEG, CairoPS, etc. require the Cairo package (optional)
# option dev is vectorized, so to set different figure sizes, do:
# dev=c('pdf', 'png'), fig.width = c(5, 2.5), fig.height = c(8, 4)
# where pdf has size 5 by 8 and png 2.5 by 4
...
# setwd("~/piketty/knitr/slides")# temp for convenience
...
### Convenience function to swap indices, e.g. palette elements
swap <- function(x,i,j) {
    x[c(i,j)] <- x[c(j,i)]
    x
} 
...
<<'Formats', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
# I have also defined sterling though it is not used in the book
# Create euro currency symbol
euro <- function(x) {
  paste0("€ ", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE))
} 
# Create sterling currency symbol 
sterling <- function(x) {
  paste0("£", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE))
} 
# Create yen currency symbol
yen <- function(x) {
  paste0("¥", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE))
} 
# Create yuan currency symbol
yuan <- yen  # not straightforward to use "元" instead of "¥"
# Create rupee currency symbol
# Rupee symbol created in 2010: ₹, unicode \u20B9 Previously: Rs
rupee <- function(x) {
  paste0("Rs", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE))
} 
# French style formatting for dollar
dollarFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE, scientific = FALSE), " $")
} 
# French style formatting for euro
euroFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE, scientific = FALSE), " €")
} 
# French style formatting for yen
yenFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE, scientific = FALSE), " ¥")
} 
# French style formatting for yuan
yuanFrench <- yenFrench
# French style formatting for sterling
yenFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE, scientific = FALSE), " £")
} 
# Create million-dollar symbol
dollarMillion <- function(x) {
  paste0("$", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE), " M")
}
# UK style formatting for million-dollar
dollarMillionUK <- function(x) {
  paste0("$", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE), " m")
} 
# Create billion-dollar symbol
dollarBillion <- function(x) {
  paste0("$", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE), "B")
}
# UK style formatting for billion-dollar symbol
dollarBillionUK <- function(x) {
  paste0("$", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE, scientific = FALSE), " bn")
} 
# French style formatting for million-dollar symbol
dollarMillionFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE, scientific = FALSE), " Mio $")
} 
# French style formatting for billion-dollar symbol
dollarBillionFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE, scientific = FALSE), " Mrd $")
} 
#  Toggle On/Off to set formats to French or US (or UK)
# e.g. dollar <- dollarFrench  overwrite the default 'dollar' styles of library(scales)
# dollar <- dollarFrench
# euro <- euroFrench
# yen <- yenFrench
# yuan <- yuanFrench
# dollarMillion <- dollarMillionFrench
# dollarBillion <- dollarBillionFrench
# dollarMillion <- dollarMillionUK
# dollarBillion <- dollarBillionUK
@
...
<<'ggplot_dual_axis', include=FALSE>>=
...
##' Known bugs: the rhs grid lines overwrite data layer + legend!
##' Problems: x-data must be compatible with scale_x_continuous()
##'   change code below to scale_x_date(), etc.. as appropriate
##'  

ggplot_dual_axis <- function(lhs, rhs, axis.title.y.rhs = "rotate") {
  # 1. Raise plot.margin to accommodate labels on the rhs
    # default values for theme_bw() are unit(c(1, 1, 0.5, 0.5), "lines")
    library(grid) # provides function unit
    lhs <- suppressMessages(lhs + theme(plot.margin = unit(c(1,3,0.5,1), "lines")))
  # 2.  Fix the right y-axis label justification
    rhs <- suppressMessages(rhs + theme(axis.text.y = element_text(hjust = 0)))
  # 3.  Rotate the right y-axis label by 270 degrees by default
  if (missing(axis.title.y.rhs) | 
	    axis.title.y.rhs %in% c("rotate", "rotated")) {
        rhs <- suppressMessages(rhs + theme(axis.title.y = element_text(angle = 270))) 
    }
  # 4.  Remove the right plot x-axis labels to prevent over-printing
    # To Do: figure out what ggplot layers are present and remove them, 
    # e.g. if x-axis has dates, use scale_x_date(NULL)
    # e.g. standard numeric, use scale_x_continuous(NULL), etc.
    # Right now you need to manually change this line
    rhs <- suppressMessages(rhs + scale_x_continuous(NULL)) 
  # 5.  Remove the right plot title to prevent over-printing
    rhs <- suppressMessages(rhs + ggtitle(NULL))
  # 6. Figure out which style is best
    # theme_bw() defaults are:
    # panel.grid.major = theme_line(colour = "grey90", size = 0.2)
    # panel.grid.minor = theme_line(colour = "grey98", size = 0.5)
  # 6a. Use only major grid lines for the left axis
    lhs <- suppressMessages(lhs + theme(panel.grid.minor = element_blank()))
  # 6b. Use only major grid lines for the right axis
  #     force transparency of the backgrounds to allow grid lines to show
    rhs <- suppressMessages(rhs + theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank(),  # prevent overplotting
...
# Process gtable objects
  ...
  # 8.  Overlap the panel of the rhs plot on that of the lhs plot
	pp <- c(subset(g1$layout, name == "panel", se = t:r))
	g <- gtable_add_grob(g1, 
	    g2$grobs[[which(g2$layout$name == "panel")]], pp$t, pp$l, pp$b, pp$l)
  # 9.  Tweak axis position, ticks and labels
	ia <- which(g2$layout$name == "axis-l")
	ax <- g2$grobs[[ia]]$children[["axis"]]  # g2$grobs[[ia]]$children[[2]]
	ax$widths <- rev(ax$widths)
	ax$grobs <- rev(ax$grobs)
	# insert space on the right of the plot to fit the second axis
	g <- gtable_add_cols(g, widths = g2$widths[g2$layout[ia, ]$l], 
	                        pos = length(g$widths)-1)
	# overlay the second axis to the right of the plot
	g <- gtable_add_grob(g, grobs = ax, 
	                        t = pp$t, l = length(g$widths)-1, b  = pp$b)
	# overlay the second label to the right of the plot
	g <- gtable_add_grob(g, grobs = g2$grobs[[7]], 
	                        t = pp$t, l = length(g$widths), b = pp$b)
	# adjust ticks alignment along the right axis
	ax$grobs[[1]]$x <- ax$grobs[[1]]$x - unit(1, "npc") + unit(0.15, "cm")
  # 10.   by-pass class check in ggsave to print a gtable with ggsave 
    ggsave <- ggplot2::ggsave; body(ggsave) <- body(ggplot2::ggsave)[-2]
  # 11.  return the gtable with grid.draw
    grid.newpage()
    return(grid.draw(g))
}
# end of ggplot_dual_axis
@ 
...
\centering
<<'Figure_0_1', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_0_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_1_1', out.width='1\\linewidth'>>=
load("data/df_1_1.Rda")  # load data from /data subdirectory
ggplot(data = df_1_1, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = FALSE) + ylab("Share in world output (% of total)") + xlab(NULL) + scale_x_continuous(limits = c(1700, 2040), breaks = seq(1700, 2020, by = 50)) + scale_y_continuous(labels = percent, breaks = seq(0, 1, by = .1)) + scale_fill_brewer(palette = fillPalette) + geom_text(data = subset(df_1_1, Year == tail(df_1_1$Year, 1)), aes(x = 3+Year, y = cumsum(value), label = variable), vjust = 1, hjust = 0, size = 4)
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_1_1b', out.width='1\\linewidth'>>=
load("data/df_1_1.Rda")  # load data from /data subdirectory
ggplot(data = df_1_1, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("Share in world output (% of total)") + xlab(NULL) + scale_x_continuous(limits = c(1700, 2012), breaks = seq(1700, 2020, by = 50)) + scale_y_continuous(labels = percent, breaks = seq(0, 1, by = .1)) + scale_fill_brewer(palette = fillPalette) + theme(legend.position = c(0.25, 0.75), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 1), fill = guide_legend(reverse = TRUE))
@
\caption{Europe's GDP made 47 percent of world GDP in 1913, down to 25 percent in 2012.}
\end{minipage}
\end{figure}
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_1_2b', out.width='1\\linewidth'>>=
load("data/df_1_2.Rda")  # load data from /data subdirectory
ggplot(data = df_1_2, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("Share in world population (% of total)") + xlab(NULL) + scale_x_continuous(limits = c(1700, 2012), breaks = seq(1700, 2010, by = 50)) + scale_y_continuous(labels = percent, breaks = seq(0, 1, by = .1)) + scale_fill_brewer(palette = fillPalette) + theme(legend.position = c(0.25, 0.75), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 1), fill = guide_legend(reverse = TRUE))
@
\caption{Europe's population made 26 percent of world population in 1913, down to 10 percent in 2012.}
\end{minipage}
\end{figure}
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_1_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_1_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_1_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S1_4a', out.width='1\\linewidth'>>=
load("data/df_S1_4a.Rda")  # load data from /data subdirectory
ggplot(data = df_S1_4a, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Exchange rate Euro/Rupee (in rupee)") + xlab(NULL) + scale_x_continuous(breaks = seq(1990, 2012, by = 2)) + scale_y_continuous(limits = c(0, 70), labels = rupee, breaks = seq(0, 70, by = 10)) + scale_shape_manual(name = "Market rate versus PPP", values = shapePalettePaired) + scale_colour_manual(name = "Market rate versus PPP", values = colorPalette) + theme(legend.position = c(.25, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S1_4b', out.width='1\\linewidth'>>=
load("data/df_S1_4b.Rda")  # load data from /data subdirectory
ggplot(data = df_S1_4b, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Exchange rate Euro/Yen (in yen)") + xlab(NULL) + scale_x_continuous(breaks = seq(1990, 2012, by = 2)) + scale_y_continuous(limits = c(95, 200), labels = yen, breaks = seq(100, 200, by = 10)) + scale_shape_manual(name = "Market rate versus PPP", values = shapePalettePaired) + scale_colour_manual(name = "Market rate versus PPP", values = colorPalette) + theme(legend.position = c(.7, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S1_5a', out.width='1\\linewidth'>>=
load("data/df_S1_5a.Rda")  # load data from /data subdirectory
ggplot(data = df_S1_5a, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Exchange rate Dollar/Yuan (in yuan)") + xlab(NULL) + scale_x_continuous(breaks = seq(1990, 2012, by = 2)) + scale_y_continuous(limits = c(2, 9), labels = yuan, breaks = seq(1, 10, by = 1)) + scale_shape_manual(name = "Market rate versus PPP", values = shapePalettePaired) + scale_colour_manual(name = "Market rate versus PPP", values = colorPalette) + theme(legend.position = c(.5, .5)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S1_5b', out.width='1\\linewidth'>>=
load("data/df_S1_5b.Rda")  # load data from /data subdirectory
ggplot(data = df_S1_5b, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Exchange rate Dollar/Rupee (in rupee)") + xlab(NULL) + scale_x_continuous(breaks = seq(1990, 2012, by = 2)) + scale_y_continuous(limits = c(0, 60), labels = rupee, breaks = seq(0, 60, by = 10)) + scale_shape_manual(name = "Market rate versus PPP", values = shapePalettePaired) + scale_colour_manual(name = "Market rate versus PPP", values = colorPalette) + theme(legend.position = c(.55, .45)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S1_5c', out.width='1\\linewidth'>>=
load("data/df_S1_5c.Rda")  # load data from /data subdirectory
ggplot(data = df_S1_5c, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Exchange rate Dollar/Yen (in yen)") + xlab(NULL) + scale_x_continuous(breaks = seq(1990, 2012, by = 2)) + scale_y_continuous(limits = c(60, 200), labels = yen, breaks = seq(60, 200, by = 20)) + scale_shape_manual(name = "Market rate versus PPP", values = shapePalettePaired) + scale_colour_manual(name = "Market rate versus PPP", values = colorPalette) + theme(legend.position = c(.75, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_1', out.width='1\\linewidth'>>=
load("data/df_2_1.Rda")  # load data from /data subdirectory
ggplot(data = df_2_1, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = FALSE) + ylab("World population (millions inhabitants)") + xlab(NULL) + scale_x_continuous(limits = c(1700, 2040), breaks = seq(1700, 2020, by = 50)) + scale_y_continuous(labels = comma, breaks = seq(0, 7000, by = 1000)) + scale_fill_brewer(palette = fillPalette) + geom_text(data = subset(df_2_1, Year == tail(df_2_1$Year, 1)), aes(x = 3+Year, y = cumsum(value), label = variable), vjust = 1, hjust = 0, size = 4)
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_1b', out.width='1\\linewidth'>>=
load("data/df_2_1.Rda")  # load data from /data subdirectory
ggplot(data = df_2_1, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("World population (millions inhabitants)") + xlab(NULL) + scale_x_continuous(limits = c(1700, 2012), breaks = seq(1700, 2020, by = 50)) + scale_y_continuous(labels = comma, breaks = seq(0, 7000, by = 1000)) + scale_fill_brewer(name = "Population", palette = fillPalette) + theme(legend.position = c(0.25, 0.75)) + guides(shape = guide_legend(ncol = 1), fill = guide_legend(reverse = TRUE))
@
\caption{World population rose from 600 million inhabitants in 1700 to 7 billion in 2012.}
\end{minipage}
\end{figure}
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_2S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_2_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_3_2', out.width='1\\linewidth'>>=
load("data/df_3_2.Rda")  # load data from /data subdirectory
ggplot(data = df_3_2, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("Value of national capital (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1700, 2020, by = 50)) + scale_y_continuous(labels = percent, breaks = seq(0, 8, by = 1)) + scale_fill_brewer(palette = fillPalette) + theme(legend.position = c(0.25, 0.25), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 1)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_3_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_3_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_3_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_3_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_6', out.width='1\\linewidth'>>=
load("data/df_4_6.Rda")  # load data from /data subdirectory
ggplot(data = df_4_6, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("Value of national capital (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1770, 2010, by = 20)) + scale_y_continuous(labels = percent, breaks = seq(0, 6, by = 1)) + scale_fill_brewer(palette = fillPalette) + theme(legend.position = c(0.15, 0.85), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 1), fill = guide_legend(reverse = TRUE))
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_7', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_8', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_9', out.width='1\\linewidth'>>=
load("data/df_4_9.Rda")  # load data from /data subdirectory
ggplot(data = df_4_9, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("Value of national capital (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1860, 2010, by = 20)) + scale_y_continuous(labels = percent, breaks = seq(0, 6, by = 1)) + scale_fill_brewer(palette = fillPalette) + theme(legend.position = c(0.75, 0.85), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 1), fill = guide_legend(reverse = TRUE))
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_10', out.width='1\\linewidth'>>=
load("data/df_4_10.Rda")  # load data from /data subdirectory
ggplot(data = df_4_10, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = TRUE) + ylab("Value of national capital (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1770, 2010, by = 20)) + scale_y_continuous(limits = c(-.3, 7), labels = percent, breaks = seq(0, 8, by = 1)) + scale_fill_brewer(palette = fillPalette) + theme(legend.position = c(.2, .83), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 1), fill = guide_legend(reverse = TRUE))
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_4_11', out.width='1\\linewidth'>>=
load("data/df_4_11.Rda")  # load data from /data subdirectory
ggplot(data = df_4_11, aes(x = Country, y = value, group = variable, fill = variable)) + geom_bar(stat = "identity", position = "stack", width = .8) + scale_fill_brewer(palette = fillPalette)  + ylab("Value of capital (% national income)") + xlab(NULL) + scale_y_continuous(labels = percent, breaks = seq(0, 7, by = 1)) + theme(legend.position = c(.86,.8), legend.title = element_blank()) + guides(fill = guide_legend(ncol = 1, reverse = FALSE))
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_3S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_4S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_5X', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_5XS', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_5', out.width='1\\linewidth'>>= ## out.height='0.75\\textheight'
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_7', out.width='1\\linewidth'>>=  ##out.height='0.75\\textheight'
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_7S', out.width='1\\linewidth'>>=  ##out.height='0.75\\textheight'
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_5_8', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S5_6', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S5_6.Rda")  # load data from /data subdirectory
ggplot(data = df_S5_6, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Foreign assets and liabilities (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1970, 2010, by = 5)) + scale_y_continuous(limits = c(-.4, 1.4), labels = percent, breaks = seq(-.4, 1.4, by = .2)) + scale_shape_manual(name = "variable", values = shapePalette) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.8), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S5_7', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S5_7.Rda")  # load data from /data subdirectory
ggplot(data = df_S5_7, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Foreign assets and liabilities (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1970, 2010, by = 5)) + scale_y_continuous(limits = c(0, 1.6), labels = percent, breaks = seq(0, 1.6, by = .2)) + scale_shape_manual(name = "variable", values = shapePalette) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.8), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S5_8', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S5_8.Rda")  # load data from /data subdirectory
ggplot(data = df_S5_8, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Foreign assets and liabilities (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1970, 2010, by = 5)) + scale_y_continuous(limits = c(0, 2.7), labels = percent, breaks = seq(0, 2.8, by = .4)) + scale_shape_manual(name = "variable", values = shapePalette) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.8), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S5_9', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S5_9.Rda")  # load data from /data subdirectory
ggplot(data = df_S5_9, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Foreign assets and liabilities (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1970, 2010, by = 5)) + scale_y_continuous(limits = c(-.2, 3.3), labels = percent, breaks = seq(-.2, 3.5, by = .4)) + scale_shape_manual(name = "variable", values = shapePalette) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.8), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S5_10', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S5_10.Rda")  # load data from /data subdirectory
ggplot(data = df_S5_10, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Foreign assets and liabilities (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1970, 2010, by = 5)) + scale_y_continuous(limits = c(-1, 9), labels = percent, breaks = seq(-1, 9, by = 1)) + scale_shape_manual(name = "variable", values = shapePalette) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.8), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S5_11', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S5_11.Rda")  # load data from /data subdirectory
ggplot(data = df_S5_11, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Foreign assets and liabilities (% national income)") + xlab(NULL) + scale_x_continuous(breaks = seq(1980, 2010, by = 5)) + scale_y_continuous(limits = c(-1.5, 3), labels = percent, breaks = seq(-1.5, 3, by = .5)) + scale_shape_manual(name = "variable", values = shapePalette) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.8), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_1', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_7', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_8', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_6_8S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_7_1X', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_3', out.width='1\\linewidth'>>=
load("data/df_8_3.Rda")  # load data from /data subdirectory
ggplot(data = df_8_3, aes(x = Quantile, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share in total income of various fractiles") + xlab(NULL) + scale_y_continuous(labels = percent, breaks = seq(0, 1, by = .1)) + scale_shape_manual(name = "France", values = shapePalettePaired) + scale_colour_manual(name = "France", values = colorPalette) + theme(legend.position = c(.6, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_4', out.width='1\\linewidth'>>=
load("data/df_8_4.Rda")  # load data from /data subdirectory
ggplot(data = df_8_4, aes(x = Quantile, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share in total income of various fractiles") + xlab(NULL) + scale_y_continuous(labels = percent, breaks = seq(0, 1, by = .1)) + scale_shape_manual(name = "France", values = shapePalettePaired) + scale_colour_manual(name = "France", values = colorPalette) + theme(legend.position = c(.6, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_5', out.width='1\\linewidth'>>=
load("data/df_8_5.Rda")  # load data from /data subdirectory
ggplot(data = df_8_5, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share of top decile in national income") + xlab(NULL) + scale_x_continuous(breaks = seq(1910, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(.3, .5, by = .05), limits = c(.3, .5)) + scale_shape_manual(name = "United States: share of top decile", values = shapePalettePaired) + scale_colour_manual(name = "United States: share of top decile", values = colorPalette) + theme(legend.position = c(.6, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_6', out.width='1\\linewidth'>>=
load("data/df_8_6.Rda")  # load data from /data subdirectory
ggplot(data = df_8_6, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share of the different groups in total income") + xlab(NULL) + scale_x_continuous(breaks = seq(1910, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(0.08, .25, by = .02), limits = c(.08, .25)) + scale_shape_manual(name = "United States: annual incomes in 2010", values = shapePalette) + scale_colour_manual(name = "United States: annual incomes in 2010", values = colorPalette) + theme(legend.position = c(.6, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_7', out.width='1\\linewidth'>>=
load("data/df_8_7.Rda")  # load data from /data subdirectory
ggplot(data = df_8_7, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share of top decile in total (incomes or wages)") + xlab(NULL) + scale_x_continuous(breaks = seq(1910, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(.2, .5, by = .05)) + scale_shape_manual(name = "United States: share of top decile", values = swap(shapePalettePaired,2,3)) + scale_colour_manual(name = "United States: share of top decile", values = swap(colorPalette,2,3)) + theme(legend.position = c(.6, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_7S', out.width='1\\linewidth'>>=
load("data/df_8_7.Rda")  # load data from /data subdirectory
ggplot(data = subset(df_8_7, variable %in% c("Incomes", "Wages")), aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share of top decile in total (incomes or wages)") + xlab(NULL) + scale_x_continuous(breaks = seq(1910, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(.2, .5, by = .05)) + scale_shape_manual(name = "United States: share of top decile", values = shapePalettePaired) + scale_colour_manual(name = "United States: share of top decile", values = colorPalette) + theme(legend.position = c(.6, .85)) 
@
\caption{The rise of income inequality since the 1970s is largely due to the rise of wage inequality.}
\end{minipage}
\end{figure}
...
\begin{minipage}[b]{\textwidth}
\centering 
<<'Figure_8_8', out.width='1\\linewidth'>>=
load("data/df_8_8.Rda")  # load data from /data subdirectory
ggplot(data = df_8_8, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share of top percentile in total (incomes or wages)") + xlab(NULL) + scale_x_continuous(breaks = seq(1910, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(0, .3, by = .02)) + scale_shape_manual(name = "United States: share of top percentile", values = swap(shapePalettePaired,2,3)) + scale_colour_manual(name = "United States: share of top percentile", values = swap(colorPalette,2,3)) + theme(legend.position = c(.6, .85)) 
@
...
\begin{minipage}[b]{\textwidth}
\centering 
<<'Figure_8_8S', out.width='1\\linewidth'>>=
load("data/df_8_8.Rda")  # load data from /data subdirectory
ggplot(data = subset(df_8_8, variable %in% c("Incomes", "Wages")), aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Share of top percentile in total (incomes or wages)") + xlab(NULL) + scale_x_continuous(breaks = seq(1910, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(0, .3, by = .02)) + scale_shape_manual(name = "United States: share of top percentile", values = shapePalettePaired) + scale_colour_manual(name = "United States: share of top percentile", values = colorPalette) + theme(legend.position = c(.6, .85)) 
@
\caption{The rise in the top 1 percent highest incomes since the 1970s is largely due to the rise in the top 1 percent highest wages.}
\end{minipage}
\end{figure}
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_9', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_8_10', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_1_data', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
...
@
<<'Figure_9_1', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S9_1', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S9_1.Rda")  # load data from /data subdirectory
ggplot(data = df_S9_1, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Hourly minimum wage") + xlab(NULL) + scale_x_continuous(breaks = seq(1950, 2013, by = 10)) + scale_y_continuous(limits = c(0, 10), labels = euro, breaks = seq(0, 10, by = 1)) + scale_shape_manual(name = "variable", values = shapePalettePaired) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.25, 0.85), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_S9_2', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
load("data/df_S9_2.Rda")  # load data from /data subdirectory
ggplot(data = df_S9_2, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Hourly minimum wage") + xlab(NULL) + scale_x_continuous(breaks = seq(1950, 2013, by = 10)) + scale_y_continuous(limits = c(0, 11), labels = dollar, breaks = seq(0, 11, by = 1)) + scale_shape_manual(name = "variable", values = shapePalettePaired) + scale_colour_manual(name = "variable", values = colorPalette) + theme(legend.position = c(0.8, 0.85), legend.title = element_blank())
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_5X', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_6', out.width='1\\linewidth'>>=
# Figure title incorrect in book, "Top decile" should read "Top thousandile"!
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_6X', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_7', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_7X', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_8', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_9_9', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_1', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_1S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_3S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_4S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_5S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_6S', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_7', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_8', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_9', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_10', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_10_11', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_1', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_2', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_5', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_7', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_8', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_9', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_10', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_11', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_11_12', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_12_1_data', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
...
@
<<'Figure_12_1', out.width='1\\linewidth'>>=
# This figure makes use of custom function ggplot_dual_axis()
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_12_2_data', results = 'hide', echo = FALSE, comment = NA, warning = FALSE, message = FALSE>>=
...
@
<<'Figure_12_2', out.width='1\\linewidth'>>=
# This figure makes use of custom function ggplot_dual_axis()
p <- ggplot_dual_axis(lhs = lhs, rhs = rhs); p
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_12_3', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_12_4', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_12_5', out.width='1\\linewidth'>>=
load("data/df_12_5.Rda")  # load data from /data subdirectory
ggplot(data = df_12_5, aes(x = Year, y = value, group = variable, fill = variable)) + geom_area(position = position_stack(reverse = TRUE), alpha = 0.7, show.legend = FALSE) + ylab("value of private capital (% world income)") + xlab(NULL) + scale_x_continuous(limits = c(1870, 2120), breaks = seq(1850, 2100, by = 30)) + scale_y_continuous(labels = percent, breaks = seq(0, 70, by = 5)) + scale_fill_brewer(palette = fillPalette) + geom_text(data = subset(df_12_5, Year == tail(df_12_5$Year, 1)), aes(x = 3+Year, y = -3+cumsum(value), label = variable), vjust = 1, hjust = 0, size = 4)
@
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_12_6', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_13_1', out.width='1\\linewidth'>>=
...
\begin{minipage}[b]{\textwidth}
\centering
<<'Figure_14_2', out.width='1\\linewidth'>>=
load("data/df_14_2.Rda")  # load data from /data subdirectory
ggplot(data = df_14_2, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 2) + ylab("Marginal tax rate applying to the highest inheritances") + xlab(NULL) + scale_x_continuous(breaks = seq(1900, 2010, by = 10)) + scale_y_continuous(labels = percent, breaks = seq(0, 1, by = .1)) + scale_shape_manual(name = "Country", values = shapePalettePaired) + scale_colour_manual(name = "Country", values = colorPalette) + theme(legend.position = c(.16, .85), legend.title = element_blank())  + guides(shape = guide_legend(ncol = 1)) 
@
...
<<'renameFigures', child='slidesFiguresRename.Rnw'>>=
@

%%% Embed fonts into all PDFs
%%% Code inserted from 'slidesFiguresEmbedFonts.Rnw'
<<'embedFonts', child='slidesFiguresEmbedFonts.Rnw'>>=
@
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
